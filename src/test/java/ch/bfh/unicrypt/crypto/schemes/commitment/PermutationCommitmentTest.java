...
//		System.out.println("g4: " + this.G_q.getRandomGenerator(ro.getRandom(4)));   // 18
	...
	public void testPermuationCommitment3() {
		...
		Permutation pi = new Permutation(new int[]{2, 0, 1});
		...
		assertTrue(commitment.getAt(1).isEqual(this.G_q.getElement(8)));   // 6^2 *13 =  8
		assertTrue(commitment.getAt(2).isEqual(this.G_q.getElement(18)));  // 6^3 * 2 = 18
	}

	@Test
	public void testPermuationCommitment4() {

		Permutation pi = new Permutation(new int[]{2, 3, 1, 0});
		...
		Tuple commitment = cp.commit(permutation, randomizations);
		assertTrue(commitment.getAt(0).isEqual(this.G_q.getElement(12)));  // 6^1 * 2 = 12
		assertTrue(commitment.getAt(1).isEqual(this.G_q.getElement(4)));   // 6^2 *18 =  8
		assertTrue(commitment.getAt(2).isEqual(this.G_q.getElement(18)));  // 6^3 * 2 = 18
		assertTrue(commitment.getAt(3).isEqual(this.G_q.getElement(12)));  // 6^4 *13 = 18
	}
