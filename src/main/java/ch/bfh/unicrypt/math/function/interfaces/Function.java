...
public interface Function {
   ...
   * @throws IllegalArgumentException if {@literal element} is null or not
   ...
   * random generator is specified, i.e., if {@literal random} is null, then the
   * system-wide random generator is taken. If the function is deterministic,
   * then {@literal random} is ignored. This is the main the method to implement
   ...
   * @throws IllegalArgumentException if the elements in {@literal elements} are
   *                                  not contained in the corresponding
   *                                  sub-domains
   * @throws IllegalArgumentException if the the length of {@literal elements}
   *                                  is different from {@literal getArityIn()}
   */
   ...
   * values. The specified elements are used to create a corresponding tuple
   * element first, which is then used to call the ordinary method
   * {@literal apply(Element element, Random random)}. In case of a randomized
   * function, a random generator can be given as an additional parameter. If no
   * random generator is specified, i.e., if {@literal random} is null, then the
   * system-wide random generator is taken. If the function is deterministic,
   * then {@literal random} is ignored.
   * <p>
   * @param elements The given input elements
   * @param random   Either {@literal null} or a given random generator
   * @return The resulting output element
   ...
   * @param index   The index of the corresponding group in the product (or
   *                power group) domain
   * @return The resulting partially applied function
   * @throws IllegalArgumentException  if {@literal element} is null or not an
   *                                   element of the corresponding sub-domain
   *                                   group
