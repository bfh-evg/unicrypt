...
public class BitArray
	...
	private BitArray(ByteArray byteArray, int arrayLength) {
		this(byteArray, arrayLength, 0, false, 0, 0, 0);
	}

	private BitArray(ByteArray byteArray, int arrayLength, int offset, boolean reverse, int trailer, int header, int length) {
		super(BitArray.class, false, arrayLength, offset, length, trailer, header, reverse);
	...
	protected BitArray abstractReverse() {
		...
		return new BitArray(this.byteArray, this.length, this.rangeOffset, !this.reverse, this.header, this.trailer, this.rangeLength);
	}

	@Override
	protected BitArray abstractGetInstance(int length, int rangeOffset, int rangeLength, int trailer, int header) {
		return new BitArray(this.byteArray, length, rangeOffset, this.reverse, trailer, header, rangeLength);
	}
