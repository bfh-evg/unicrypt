...
public class FilteredSequence<V>
	...
	protected FilteredSequence(Iterable<V> values, Predicate<V> predicate) {
		super(computeLength(Sequence.UNKNOWN));
		this.values = values;
		this.predicate = predicate;
	}

	protected FilteredSequence(Collection<V> collection, Predicate<V> predicate) {
		super(computeLength(BigInteger.valueOf(collection.size())));
		this.values = collection;
		this.predicate = predicate;
	}

	protected FilteredSequence(Sequence<V> sequence, Predicate<V> predicate) {
		super(computeLength(sequence.getLength()));
		this.values = sequence;
		this.predicate = predicate;
	}

	private static BigInteger computeLength(BigInteger length) {
		if (length.signum() == 0) {
			return length;
		}
