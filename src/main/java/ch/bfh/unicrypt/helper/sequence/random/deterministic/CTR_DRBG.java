 ...
 * random oracles, but should not be used as pseudo-random generators.
 * <p>
 * @author R. Haenni
 * @version 2.0
 * @see RandomOracle
...
public class CTR_DRBG
	...
	private CTR_DRBG(HashAlgorithm hashAlgorithm, ByteArray seed) {
		super(seed);
		this.hashAlgorithm = hashAlgorithm;
		this.converter = ByteArrayToBigInteger.getInstance(seed.getLength());
	}
	...
	public RandomByteArraySequenceIterator iterator() {
		...
		return new RandomByteArraySequenceIterator() {
			...
			private BigInteger counter = converter.convert(seed);

			@Override
			public ByteArray abstractNext() {
				ByteArray next = hashAlgorithm.getHashValue(converter.reconvert(this.counter));
				this.counter = this.counter.add(MathUtil.ONE).mod(MathUtil.powerOfTwo(hashAlgorithm.getBitLength()));
				return next;
			}
	...
	public static DeterministicRandomByteArraySequence.Factory getFactory() {
		return CTR_DRBG.getFactory(HashAlgorithm.getInstance());
	}
	...
	public static DeterministicRandomByteArraySequence.Factory getFactory(final HashAlgorithm hashAlgorithm) {
		...
		return new DeterministicRandomByteArraySequence.Factory() {
			...
			protected DeterministicRandomByteArraySequence abstractGetInstance(ByteArray seed) {
				return new CTR_DRBG(hashAlgorithm, seed);
			}

			@Override
			protected int getSeedLength() {
				return hashAlgorithm.getByteLength();
			}
